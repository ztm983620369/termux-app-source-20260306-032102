...
class Breadcrumbs(context: Context, attrs: AttributeSet) : HorizontalScrollView(context, attrs) {
    private companion object {
        private const val BUBBLE_FILL_ALPHA = 0.12f
    }
    ...
    var isRootBreadcrumbVisible = true
        set(value) {
            field = value
    ...
    private fun addBreadcrumb(item: FileDirItem, index: Int, isLast: Boolean) {
        ItemBreadcrumbBinding.inflate(inflater, itemsLayout, false).apply {
            ...
            val itemSpacing = context.resources.getDimensionPixelSize(R.dimen.small_margin)
            ...
            itemsLayout.addView(root, layoutParams)

            val horizontalPadding = context.resources.getDimensionPixelSize(R.dimen.normal_margin)
            breadcrumbText.updatePaddingRelative(start = horizontalPadding, end = horizontalPadding)
            ...
            if (index == 0) {
                ...
                if (!isRootBreadcrumbVisible) {
                    root.visibility = View.GONE
                }
            } else {
                setupBubbleBreadcrumbBackground(breadcrumbText)
            }
