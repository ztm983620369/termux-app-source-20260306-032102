...
abstract public class BasePluginProcessService extends Service {
     ...
     */
    private static BasePluginProcessService sSingleInstanceOwner = null;

    @Override
    public void onCreate() {
        synchronized (BasePluginProcessService.class) {
            if (sSingleInstanceOwner == null) {
                sSingleInstanceOwner = this;
            } else if (sSingleInstanceOwner != this && mLogger.isWarnEnabled()) {
                mLogger.warn("PPS duplicate instance ignored. current:{} new:{}",
                        sSingleInstanceOwner, this);
            }
        }
        super.onCreate();
    ...
    public void onDestroy() {
        super.onDestroy();
        synchronized (BasePluginProcessService.class) {
            if (sSingleInstanceOwner == this) {
                sSingleInstanceOwner = null;
            }
        }
        if (mLogger.isInfoEnabled()) {
