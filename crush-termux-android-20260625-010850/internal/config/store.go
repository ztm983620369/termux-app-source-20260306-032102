...
// RemoveConfigField) to prevent both in-process goroutine races and,
// together with the shared lock.File, cross-process races on the config file.
//
...
// RefreshOAuthToken is intentionally disabled in the Termux build.
func (s *ConfigStore) RefreshOAuthToken(ctx context.Context, scope Scope, providerID string) error {
	_, _ = ctx, scope
	return fmt.Errorf("official OAuth refresh has been removed for provider %s", providerID)
}

// loadTokenFromDisk reads the OAuth token for the given provider from the
// config file on disk. Returns nil if the token is not found or has no access
// token.
