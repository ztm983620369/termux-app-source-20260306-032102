...
func InitializeSingle(ctx context.Context, name string, cfg *config.ConfigStore) error {
	...
	if !exists {
		return fmt.Errorf("配置中找不到 MCP %q", name)
	}
...
func getOrRenewClient(ctx context.Context, cfg *config.ConfigStore, name string) (*ClientSession, error) {
	...
	if !ok {
		return nil, fmt.Errorf("MCP %q 不可用", name)
	}
...
func maybeTimeoutErr(err error, timeout time.Duration) error {
	if errors.Is(err, context.Canceled) {
		return fmt.Errorf("%s 后超时", timeout)
	}
	return err
}
...
func createTransport(ctx context.Context, m config.MCPConfig, resolver config.VariableResolver) (mcp.Transport, error) {
	...
	case config.MCPStdio:
		...
		if err != nil {
			return nil, fmt.Errorf("MCP 命令无效：%w", err)
		}
		if strings.TrimSpace(command) == "" {
			return nil, fmt.Errorf("MCP stdio 配置需要非空的 'command' 字段")
		}
	...
	case config.MCPHttp:
		...
		if strings.TrimSpace(url) == "" {
			return nil, fmt.Errorf("MCP http 配置需要非空的 'url' 字段")
		}
	...
	case config.MCPSSE:
		...
		if strings.TrimSpace(url) == "" {
			return nil, fmt.Errorf("MCP sse 配置需要非空的 'url' 字段")
		}
		headers, err := m.ResolvedHeaders(resolver)
		if err != nil {
			return nil, err
	...
	default:
		return nil, fmt.Errorf("不支持的 MCP 类型：%s", m.Type)
	}
}
