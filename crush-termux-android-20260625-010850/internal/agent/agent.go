...
func (a *sessionAgent) Run(ctx context.Context, call SessionAgentCall) (result *fantasy.AgentResult, retErr error) {
	...
	if shouldSummarize {
		...
		if len(currentAssistant.ToolCalls()) > 0 {
			...
			call.Prompt = fmt.Sprintf("上一段会话因为过长而被中断，最初的用户请求是：`%s`", call.Prompt)
...
func (a *sessionAgent) createUserMessage(ctx context.Context, call SessionAgentCall) (message.Message, error) {
	...
	if err != nil {
		return message.Message{}, fmt.Errorf("创建用户消息失败：%w", err)
	}
	return msg, nil
}
	...
	for _, tc := range m.ToolCalls() {
		...
		syntheticParts = append(syntheticParts, fantasy.ToolResultPart{
			...
			Output: fantasy.ToolResultOutputContentError{
				Error: errors.New("工具调用被中断且没有生成结果；如果仍需要结果，可以重试此调用"),
			},
		})
	}
...
func (a *sessionAgent) getSessionMessages(ctx context.Context, session session.Session) ([]message.Message, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("列出消息失败：%w", err)
	}
...
func (a *sessionAgent) GenerateTitle(ctx context.Context, sessionID string, userPrompt string) {
	...
	streamCall := fantasy.AgentStreamCall{
		Prompt: fmt.Sprintf("请为以下内容生成一个简短标题：\n\n%s\n <think>\n\n</think>", userPrompt),
	...
	case fantasy.ToolResultContentTypeMedia:
		if r, ok := fantasy.AsToolResultOutputType[fantasy.ToolResultOutputContentMedia](result.Result); ok {
			if !stringext.IsValidBase64(r.Data) {
				...
				content := r.Text
				if content == "" {
					content = fmt.Sprintf("已加载 %s 内容", r.MediaType)
				}
...
func buildSummaryPrompt(todos []session.Todo) string {
	...
	if len(todos) > 0 {
		...
		sb.WriteString("\n请在总结中包含这些任务及其状态。")
		sb.WriteString("请指示恢复会话的助手继续使用 `todos` 工具跟踪这些任务的进度。")
	}
	return sb.String()
}
