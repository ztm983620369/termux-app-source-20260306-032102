...
import (
	...
	"github.com/charmbracelet/crush/internal/nativeext"
...
// Coordinator errors.
var (
	errCoderAgentNotConfigured         = errors.New("代码智能体未配置")
	errModelProviderNotConfigured      = errors.New("模型服务商未配置")
	errLargeModelNotSelected           = errors.New("未选择大模型")
	errSmallModelNotSelected           = errors.New("未选择小模型")
	errLargeModelProviderNotConfigured = errors.New("大模型服务商未配置")
	errSmallModelProviderNotConfigured = errors.New("小模型服务商未配置")
	errLargeModelNotFound              = errors.New("服务商配置中未找到大模型")
	errSmallModelNotFound              = errors.New("服务商配置中未找到小模型")
)
...
func (c *coordinator) buildTools(ctx context.Context, agent config.Agent, isSubAgent bool) ([]fantasy.AgentTool, error) {
	...
	allTools = append(allTools, nativeext.Tools(nativeext.Deps{
		Sessions:         c.sessions,
		CurrentSessionID: tools.GetSessionFromContext,
	})...)
...
func (c *coordinator) buildOpenaiCompatProvider(baseURL, apiKey string, headers map[string]string, extraBody map[string]any, _ string, _ bool) (fantasy.Provider, error) {
...
func (c *coordinator) buildProvider(providerCfg config.ProviderConfig, model config.SelectedModel, isSubAgent bool) (fantasy.Provider, error) {
	...
	default:
		...
		return nil, fmt.Errorf("不支持的服务商类型：%q", providerCfg.Type)
	}
}
...
func (c *coordinator) refreshOAuth2Token(ctx context.Context, providerCfg config.ProviderConfig) error {
	_ = ctx
	return fmt.Errorf("服务商 %s 的官方 OAuth 刷新已移除", providerCfg.ID)
}
...
func (c *coordinator) runSubAgent(ctx context.Context, params subAgentParams) (fantasy.ToolResponse, error) {
	...
	if err != nil {
		return fantasy.ToolResponse{}, fmt.Errorf("创建会话失败：%w", err)
	}
	...
	if err != nil {
		return fantasy.NewTextErrorResponse(fmt.Sprintf("生成响应失败：%s", err)), nil
	}
	...
	if output == "" {
		return fantasy.NewTextErrorResponse("子智能体已完成，但没有生成文本输出。"), nil
	}
