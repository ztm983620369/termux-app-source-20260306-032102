...
func TestRunSubAgent(t *testing.T) {
	...
	t.Run("nil result returns error response", func(t *testing.T) {
		...
		assert.Equal(t, "子智能体已完成，但没有生成文本输出。", resp.Content)
	})
	...
	t.Run("provider not configured", func(t *testing.T) {
		...
		require.Error(t, err)
		assert.Contains(t, err.Error(), "模型服务商未配置")
	})
	...
	t.Run("agent run error returns error response", func(t *testing.T) {
		...
		require.NoError(t, err)
		assert.True(t, resp.IsError)
		assert.Equal(t, "生成响应失败：provider request failed", resp.Content)
	})
...
func TestUpdateParentSessionCost(t *testing.T) {
	...
	t.Run("child session not found", func(t *testing.T) {
		...
		require.Error(t, err)
		assert.Contains(t, err.Error(), "获取子会话失败")
	})
	...
	t.Run("parent session not found", func(t *testing.T) {
		...
		require.Error(t, err)
		assert.Contains(t, err.Error(), "获取父会话失败")
	})
