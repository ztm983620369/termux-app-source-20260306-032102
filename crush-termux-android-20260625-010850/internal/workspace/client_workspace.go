...
func (w *ClientWorkspace) RenameSession(ctx context.Context, sessionID, title string) (session.Session, error) {
	saved, err := w.client.RenameSession(ctx, w.workspaceID(), sessionID, title)
	if err != nil {
		return session.Session{}, err
	}
	return protoToSession(*saved), nil
